...
`frontend/tests/e2e/competitionLifecycle.spec.ts` 使用單一 test 與具名 `test.step`，由帳號 fixture 經 UI 建立同一場比賽。Checkpoint 6 已通過：兩組資格賽、個人八強至頒牌、第 8／9 名邊界及公開組別切換。團體、裁判更正與重啟驗收尚待後續 checkpoint，不因 helper 存在而視為完成。
...
| 團體賽目標 | 各四隊，以資格名次 `[1,5,9]`、`[2,6,10]`、`[3,7,11]`、`[4,8,12]` 組成 |
